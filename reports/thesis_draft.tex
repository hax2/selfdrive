...
An alternative line of work derives supervision from robot trajectories,
interventions, or geometric sensors. Those signals can adapt perception to a
specific platform, although their labels depend on vehicle interaction,
coverage, and sensor assumptions. This thesis uses supervised segmentation so
all architectures receive identical pixel labels and can be compared through a
...
RELLIS-3D~\cite{jiang2021rellis}     & 2021 & 6{,}235 & 20 & Off-road military proving ground \\
...
RELLIS-3D also provides 13{,}556 annotated LiDAR scans; the corrected image
count in Table~\ref{tab:offroad_datasets} is 6{,}235. WildScenes
~\cite{vidanapathirana2025wildscenes} extends natural-environment benchmarking
with calibrated image and LiDAR sequences collected across repeated traversals
over six months, supporting 2D, 3D, and domain-adaptation evaluation. These
larger multimodal datasets offer stronger tests of environmental generalisation
than CaT, although their semantic terrain labels do not reproduce CaT's nested
...
The models evaluated in this thesis represent three important design families:
generic encoder--decoder systems, purpose-built real-time multi-resolution
networks, and compact transformer-based segmentation.

\subsection{Encoder--Decoder Systems and Compact Encoders}

U-Net~\cite{ronneberger2015unet} introduced a contracting path followed by a
symmetric expanding path, with skip connections restoring spatial detail from
matching encoder stages. Its decoder remains widely used because it can be
paired with different feature encoders. FPN~\cite{lin2017fpn} instead constructs
a top-down pyramid and merges it with lateral features, producing semantically
strong representations at several spatial scales. Although introduced for
object detection, FPN is also used as a compact semantic-segmentation decoder.

MobileNetV2~\cite{sandler2018mobilenetv2} reduces computation through depthwise
convolutions, inverted residual blocks, and linear bottlenecks. EfficientNet
~\cite{tan2019efficientnet} scales depth, width, and resolution jointly;
EfficientNet-B0 is its smallest baseline. Pairing both encoders with both
decoders yields the $2\times2$ design used here. This design distinguishes
decoder effects from encoder effects, which a comparison of two unmatched
encoder--decoder packages cannot do.

\subsection{Purpose-Built Real-Time Networks}

BiSeNetV2~\cite{yu2021bisenetv2} assigns fine spatial detail and high-level
semantics to separate branches, then combines them through guided aggregation.
DDRNet~\cite{hong2022ddrnet} maintains coupled high- and low-resolution streams
with repeated bilateral exchange; DDRNet-23-Slim targets the high-throughput
end of that family. PIDNet~\cite{xu2023pidnet} extends multi-branch reasoning
with an analogy to proportional, integral, and derivative control:
\begin{itemize}[nosep]
  \item the P branch preserves high-resolution spatial detail;
  \item the I branch aggregates low-resolution semantic context;
  \item the D branch emphasises boundaries and corrects detail loss during fusion.
\end{itemize}
PIDNet-S uses approximately 7.6M parameters. Its paper reports 78.6\% mIoU on
the Cityscapes test set at 93.2 FPS under the authors' measurement setup. Such
published FPS values depend on hardware, resolution, software, and timing
scope, so this thesis remeasures every candidate under common H100 and Ryzen
protocols instead of comparing literature FPS directly.

\subsection{Compact Transformer Segmentation}

SegFormer~\cite{xie2021segformer} uses a hierarchical Mix Transformer encoder
and a lightweight MLP decoder. Its encoder avoids fixed positional encodings,
and its multi-scale features provide both local detail and broader context.
The B0 variant is included as the compact transformer candidate. Transformer
parameter count alone does not determine deployment speed; attention, tensor
layout, memory access, and runtime optimisation can substantially affect CPU
and GPU latency. This motivates measuring eager and compiled CPU execution in
addition to server-GPU throughput.
...
The focused literature review identified three publications that report
quantitative CaT segmentation results and provide the most directly relevant
baselines for this thesis. This count describes the reviewed comparison set
rather than claiming an exhaustive census of every use of CaT.
...
\section{Research Gap and Position of This Thesis}

The reviewed CaT studies use different class merges, model capacities, data
counts, split descriptions, and metric definitions. The original paper reports
four-class PSPNet baselines; the EfficientViT study reports three-class means
under a split that cannot be reconstructed from a public manifest; and the
depth-aided study reports a permissive merged-road class after training on a
much larger generated dataset. These results provide context, although they do
not answer which compact architecture gives the strongest safety--accuracy--
latency trade-off under one fixed CaT protocol.

This thesis addresses that gap through nine model families, two explicit
vehicle-capability policies, three matched seeds, one deterministic split, and
common safety metrics. The complete FPN/U-Net by MobileNetV2/EfficientNet-B0
design supports component-level analysis inside the generic encoder--decoder
family. Hardware-matched H100 measurements and eager/compiled Ryzen CPU
measurements establish deployment trade-offs without borrowing incompatible
FPS values from architecture papers. The contribution is an empirical CaT
benchmark with reproducible artifacts; conclusions are restricted to this
...
\subsection{Comparative Real-Time Architectures}

Seven additional compact segmentation systems broaden the comparison beyond
PIDNet-S and ROD:
\begin{itemize}[nosep]
  \item \textbf{FPN}~\cite{lin2017fpn} combines a top-down feature pyramid with
  lateral encoder connections. It is paired separately with MobileNetV2 and
  EfficientNet-B0.
  \item \textbf{U-Net}~\cite{ronneberger2015unet} uses a symmetric decoder and
  skip connections from encoder stages at matching spatial scales. It uses the
  same two encoders, completing a $2\times2$ encoder--decoder design.
  \item \textbf{MobileNetV2}~\cite{sandler2018mobilenetv2} uses inverted
  residual blocks and depthwise separable convolutions.
  \item \textbf{EfficientNet-B0}~\cite{tan2019efficientnet} is the smallest
  member of the EfficientNet family and provides a higher-capacity compact
  encoder.
  \item \textbf{SegFormer-B0}~\cite{xie2021segformer} combines a MiT-B0
  hierarchical transformer encoder with a lightweight MLP decoder.
  \item \textbf{DDRNet-23-Slim}~\cite{hong2022ddrnet} maintains high- and
  low-resolution branches with repeated bilateral fusion.
  \item \textbf{BiSeNetV2}~\cite{yu2021bisenetv2} separates spatial detail and
  semantic context into two branches before guided aggregation.
\end{itemize}

Table~\ref{tab:model_implementations} records the exact implementation and
initialisation of each two-class model. FPN, U-Net, and SegFormer use
\texttt{segmentation-models-pytorch}; DDRNet-23-Slim and BiSeNetV2 are adapted
from released PyTorch implementations under their MIT licences.

\begin{table}[H]
\centering
\caption{Compared model implementations and initialisation.}
\label{tab:model_implementations}
\small
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Model} & \textbf{Parameters} & \textbf{Initialisation} \\
\midrule
PIDNet-S                    & 7.62M  & Random \\
ROD ViT-S                   & 29.11M & EfficientSAM ViT-S; encoder fixed \\
FPN/MobileNetV2             & 4.22M  & ImageNet encoder \\
FPN/EfficientNet-B0         & 5.76M  & ImageNet encoder \\
U-Net/MobileNetV2           & 6.63M  & ImageNet encoder \\
U-Net/EfficientNet-B0       & 6.25M  & ImageNet encoder \\
SegFormer-B0                & 3.71M  & ImageNet MiT-B0 encoder \\
DDRNet-23-Slim              & 5.69M  & Random \\
BiSeNetV2                   & 3.34M  & Released pretrained backbone \\
\bottomrule
\end{tabular}
\end{table}

Initialisation is part of each evaluated recipe and is reported because it can
affect performance independently of architecture. The comparison therefore
ranks complete trainable systems. BiSeNetV2 auxiliary heads are disabled so
every controlled model is optimised through one full-resolution two-class
logits tensor and the common loss.
...
The controlled architecture comparison applies this recipe to all nine model
families for seeds 1337, 2027, and 4242 under both label policies. Within each
policy, every model uses the same 1,002/266/544 split, $640\times384$ input,
batch size 4, 15 epochs, augmentation, class-weight bias, objective, threshold,
and seed set. Model selection uses validation mIoU. The SMP $2\times2$ subset
holds one component fixed while changing the other: FPN versus U-Net for each
encoder, and MobileNetV2 versus EfficientNet-B0 for each decoder. This supports
separate analysis of encoder, decoder, and pairing effects within that subset.
...
For every controlled result, the artifact chain is:
\begin{itemize}[nosep]
  \item the policy-specific processed-data manifest;
  \item a complete base or generated seed configuration;
  \item the corresponding \texttt{checkpoints/best.pt};
  \item evaluator and metric definitions: \texttt{src/ttfm/eval.py} and
        \texttt{src/ttfm/metrics.py};
  \item the corresponding \texttt{test\_metrics.json}.
\end{itemize}
The blue-only comparison is indexed by
\texttt{reports/blue\_only\_verified\_ledger.json}, with a readable table in
\texttt{reports/blue\_only\_verified\_summary.md}. The ledger contains all 27
model--seed records, configuration, metric, checkpoint, and benchmark SHA-256
digests, per-seed confusion counts, and aggregate statistics. Its generator
recomputes each reported metric directly from TP, FP, TN, and FN; verifies
seeds 1337, 2027, and 4242; and checks that repeat configurations differ only
in seed and experiment identifier. All 27 blue-only metric-integrity checks,
seed-control checks, and checkpoint-hash checks pass. A single run can be
re-evaluated with:
\begin{verbatim}
PYTHONPATH=src python -m ttfm.cli \
  --config <exact-config.yaml> eval --split test
\end{verbatim}
The same procedure is applied to the blue+green campaign before its values
enter a thesis table. Aggregate tables are generated only after every
constituent JSON report passes these checks.
...
The experiments are organised in five phases:
\begin{enumerate}[nosep]
  ...
  \item \textbf{Nine-model architecture comparison}: PIDNet-S, ROD,
  FPN/MobileNetV2, FPN/EfficientNet-B0, U-Net/MobileNetV2,
  U-Net/EfficientNet-B0, SegFormer-B0, DDRNet-23-Slim, and BiSeNetV2 for seeds
  1337, 2027, and 4242 under both binary label policies.
  \item \textbf{SMP factorial analysis}: the full FPN/U-Net by
  MobileNetV2/EfficientNet-B0 $2\times2$ design.
...
CPU latency for the lightweight architectures was measured on an AMD Ryzen~5
5500 (6 cores, 12 threads) using PyTorch 2.13, six inference threads, batch size
1, and $640\times384$ input. Each model processes 32 preloaded test images over
five measured repeats after two excluded warm-up rounds, giving 160 timed
inferences. The machine remained idle during the retained pass. Each model is
measured in eager mode and again after \texttt{torch.compile}; compilation is
triggered during warm-up and excluded from timed latency. These values measure
forward pass and prediction only, excluding image decoding, camera acquisition,
visualisation, and disk output.

All nine models are additionally measured on the same NVIDIA H100 NVL
3g.47GB MIG partition. The server protocol uses eager PyTorch, batch size 1,
128 preloaded test images, five warm-up passes, ten timed repeats, and explicit
CUDA synchronisation around every forward pass. Label policy does not change
model structure, so latency is measured once per architecture and reused across
the blue-only and blue+green accuracy tables. The H100 results control hardware
within the architecture comparison and do not represent embedded-GPU latency.
...
edge inference process for the previously deployed PIDNet-S and ROD models.
...
34, pages 12077--12090, 2021.

\bibitem[Hong et~al.(2022)]{hong2022ddrnet}
Y.~Hong, H.~Pan, W.~Sun, and Y.~Jia.
\newblock Deep Dual-Resolution Networks for Real-Time and Accurate Semantic
Segmentation of Road Scenes.
\newblock \emph{IEEE Transactions on Intelligent Transportation Systems},
23(10):19482--19494, 2022.

\bibitem[Yu et~al.(2021)]{yu2021bisenetv2}
C.~Yu, C.~Gao, J.~Wang, G.~Yu, C.~Shen, and N.~Sang.
\newblock BiSeNet V2: Bilateral Network with Guided Aggregation for Real-Time
Semantic Segmentation.
\newblock \emph{International Journal of Computer Vision}, 129:3051--3068, 2021.

\bibitem[Vidanapathirana et~al.(2025)]{vidanapathirana2025wildscenes}
K.~Vidanapathirana, J.~Knights, S.~Hausler, M.~Cox, M.~Ramezani,
J.~Jooste, E.~Griffiths, S.~Mohamed, S.~Sridharan, C.~Fookes, and
P.~Moghadam.
\newblock WildScenes: A Benchmark for 2D and 3D Semantic Segmentation in
