...
Table~\ref{tab:edge_inference} measures the frozen blue+green deployment
candidates on a local NVIDIA GeForce RTX~5060 with actual FP16 parameters and
batch size 1. The forward scope includes GPU inference, softmax, and threshold.
...
\begin{table}[H]
\centering
\caption{FP16 batch-1 edge-style performance on the local RTX 5060.}
...
\midrule
PIDNet-S & 8.37 ms & 119.4 & 14.90 ms & 67.1 & 14.5 MiB & 57.9 MiB \\
ROD      & 31.39 ms & 31.9 & 38.36 ms & 26.1 & 55.5 MiB & 680.0 MiB \\
\bottomrule
\end{tabular}
\end{table}

Forward P95 latency is 10.55~ms for PIDNet-S and 31.63~ms for ROD; pipeline
P95 is 18.56 and 41.07~ms, respectively. FP16 therefore narrows ROD's latency
penalty to 3.75 times for forward inference and 2.58 times for the measured
pipeline, but ROD still falls below a sustained 30~FPS pipeline target. PIDNet-S
has substantially more timing and memory headroom. ROD's peak allocation is
11.75 times larger, although 680~MiB remains feasible on the tested 8~GB GPU.

Reduced precision does not explain the accuracy difference. Full 544-image
FP16 evaluation gives ROD 0.923574 mIoU versus 0.923569 in the saved FP32 suite
(a $+0.000006$ difference attributable to numerical rounding). PIDNet-S gives
0.893964 versus 0.893941 ($+0.000023$). Thus FP16 preserves the selected
operating points for both checkpoints. These figures represent an executable
PyTorch deployment baseline, not an optimised TensorRT result or a guarantee
...
\begin{enumerate}
  ...
  \item \textbf{The accuracy gain has a large latency cost}: on the same H100
  ...
  directly portable to edge hardware. On the local FP16 RTX~5060 pipeline, the
  gap narrows to 38.36 vs. 14.90~ms, but ROD remains below 30~FPS.
...
post-hoc analyses of frozen checkpoints.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Edge-style measurement completed}: both frozen model families
...
\begin{itemize}[nosep]
  ...
  \item \textbf{ONNX/TensorRT export}: optimise the frozen model and benchmark
